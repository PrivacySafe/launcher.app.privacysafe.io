...
import { getCachedAppFiles, userSystem } from '@/common/services';
import { CachedAppLaunchers } from '@/common/services/cached-system-info';
...
export function makeAppsAndLaunchersInfoPlace() {
  ...
  async function fetchAppsInfo(platform: PlatformInfo) {
    const { cacheTS: dataTS, launchers, apps } = await userSystem.getAppsInfoAndLaunchers(true);
  ...
  async function updateAppInfos(apps: AppInfo[], diffVersions: { [appId: string]: string }): Promise<void> {
    ...
    for (const app of apps) {
      applicationsInSystem.value.push(await appInfoFrom(app, diffVersions[app.appId]));
    }
    // beautify
    applicationsInSystem.value.sort(byName);
    const contactsAppInd = applicationsInSystem.value.findIndex(({ appId }) => appId === 'contacts.app.privacysafe.io');
    if (contactsAppInd > 0) {
  ...
  async function updateAppLaunchers(
    ...
    for (let i = 0; i < appLaunchers.value.length; i += 1) {
      ...
      if (indOfApp < 0) {
        ...
        appLaunchers.value[i] = await appLaunchersFromInfo(launchersInfo, diffVersions[appId]);
      }
    }
    // put the rest of launchers array into appLaunchers as these are new
    // launcher entries.
    for (const info of launchers) {
      appLaunchers.value.push(await appLaunchersFromInfo(info, diffVersions[info.appId]));
    }
    // beautify
...
function areVersionsSame(v1: AppInfo['versions'], v2: AppInfo['versions']): boolean {
  if (v1.current !== v2.current || v1.bundled !== v2.bundled || v1.latest !== v2.latest) {
    return false;
  }
  if (!Array.isArray(v1.packs)) {
    return !Array.isArray(v2.packs);
  } else if (!Array.isArray(v2.packs) || v1.packs.length !== v2.packs.length) {
    return false;
  }
...
async function appInfoFrom(info: AppInfo, diffRunningVersion: string | undefined): Promise<AppInfo> {
  const { appId, name, description, icon, versions } = info;
  const iconBytes = icon ? await getCachedAppFiles(appId, versions.latest, icon) : undefined;
  return {
    appId,
    name,
    description,
...
async function appLaunchersFromInfo(
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  diffRunningVersion: string | undefined,
): Promise<AppLaunchers> {
  const { appId, version, name, description, icon } = info;
  const iconBytes = icon ? await getCachedAppFiles(appId, version, icon) : undefined;
...
async function captureRunningAppsWithNonCurrentVersion(apps: AppInfo[]): Promise<{ [appId: string]: string }> {
