...
export function makeFileExceptionFromCode(code: string | undefined, path: string, cause?: any): FileException {
...
export function makeNoAttrsExc(path: string): FileException {
  return makeRuntimeException<FileException>('file', { path }, { attrsNotEnabledInFS: true });
}

export function makeVersionMismatchExc(path: string): FileException {
  return makeRuntimeException<FileException>('file', { path }, { versionMismatch: true });
}
