...
import { UPDATE_INFO_CACHE_TTL_SECONDS } from "@/constants";
...
export class AppDistInfoChecker {
  ...
  async getAppDistInfo(
    appId: string, forceInfoDownload = false
  ): Promise<CachedAppDistributionInfo|undefined> {
    const cachedInfo = await this.getCachedAppData(appId);
    if (!forceInfoDownload
    && cachedInfo
    && cacheIsRecent(cachedInfo.cacheTS, Date.now())) {
      return cachedInfo;
    }
    return this.procSync.startOrChain(appId, async () => {
  ...
  async getBundleDistInfo(
    ...
    return this.procSync.startOrChain(UPDATE_INFO_FILE, async () => {
...
async function downloadAppData(
  appId: string
): Promise<CachedAppDistributionInfo> {
  const downloader = w3n.system!.apps!.downloader!;
  const data = {
    appId, versions: {}, cacheTS: Date.now()
